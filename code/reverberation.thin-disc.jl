...
thick_freq1, thick_τ1, thick_impulse1, thick_time1 = calculate_lag_transfer(m, thick_d, model1, radii, thick_itb)
thick_freq2, thick_τ2, thick_impulse2, thick_time2 = calculate_lag_transfer(m, thick_d, model2, radii, thick_itb)
thick_freq3, thick_τ3, thick_impulse3, thick_time3 = calculate_lag_transfer(m, thick_d, model3, radii, thick_itb)
thick_freq4, thick_τ4, thick_impulse4, thick_time4 = calculate_lag_transfer(m, thick_d, model4, radii, thick_itb)
...
begin
    ...
    ax1 = Axis(ga[2,1], yscale = log10, xlabel = L"Time $t$", ylabel=L"Impulse Response$$")
    ax2 = Axis(ga[3,1], xscale = log10, xminorgridvisible = true, xminorticks = IntervalsBetween(10), xlabel = L"Lag $\tau$", ylabel = L"Frequency $f$")
    ...
    lines!(ax2, thick_freq1, thick_τ1, label = _format_model(model1), linestyle = :dot, color = color)
    ...
    lines!(ax2, thick_freq2, thick_τ2, label = _format_model(model2), linestyle = :dot, color = color)
    ...
    lines!(ax2, thick_freq3, thick_τ3, label = _format_model(model3), linestyle = :dot, color = color)
    ...
    lines!(ax2, thick_freq4, thick_τ4, label = _format_model(model4), linestyle = :dot, color = color)
...
continuum_time(KerrMetric(1.0, 0.998), SVector(0.0, 1000.0, deg2rad(45), 0.0), LampPostModel(h = 100.0))
continuum_time(Gradus.SphericalMetric(), SVector(0.0, 1000.0, deg2rad(45), 0.0), LampPostModel(h = 100.0))
continuum_time(Gradus.SphericalMetric(), SVector(0.0, 1000.0, deg2rad(45), 0.0), LampPostModel(h = 2.0))