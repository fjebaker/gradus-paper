...
direct image plane binning. This particularly useful for the thick disc case,
where obscuration effects make calculating the Cunningham transfer functions
additionally difficult, as described in \ref{sec:partially-obscured-functions}.
In Figure \ref{fig:line-profile-ssd} we show line profiles for the
\citet{shakura_black_1973} disc (SSD) model using a fixed power-law emissivity
function. Both methods yield identical line profiles.
...
of the code by implementing the Einstein-Maxwell-Dilaton-Axion metric derived in
\citet{garcia_class_1995} and calculate lineprofiles for various values of the
$b$ constant, shown in Figure \ref{fig:reflection-emda}.
...
\Gradus is new a open-source and publicly available general relativistic
...
software for comparisons, and thank Corbin Taylor for making the \texttt{fenrir}
