...
General relativistic ray-tracing (GRRT) is the computational technique of
integrating the trajectory of particles and photons in curved spacetimes. It
enables the calculation of spectral components and radiative processes in the
strong gravity regime around black holes, neutron stars, and other compact
objects, making the technique invaluable for models of the inner regions of the
accretion flow onto such objects. The general relativistic (GR) effects cause
significant deviations in the spectral energetics
...
\citep[e.g.][]{luminet_image_1979}. It is therefore important that GR effects
are included in spectral models of accretion onto compact objects.

GRRT has a long history in spectral modelling, with widespread
use in models of reflection spectra \citep[e.g.][]{fabian_x-ray_1989}, the
effect on the accretion disc emissivity of the X-ray
corona
\citep[e.g.][]{wilkins_understanding_2012, wilkins_towards_2016} emission and
absorption features \citep[e.g.][]{ruszkowski_absorption_2002}, winds
\citneeded, spectral energy densities of
quasars \citep[e.g.][]{hagen_estimating_2023}, in variability studies such as
reverberation lags from close to the black hole
...
X-ray reverberation, and the accompanying models use GRRT.

A favourite method for including GR effects in a model is to convolve a
rest-frame spectrum with a something akin to a \emph{line profile}
\citep{fabian_x-ray_1989, laor_line_1991}, using models such as \texttt{relline}
/ \texttt{relxill} \citep{dauser_broad_2010, dauser_relativistic_2016} or
\texttt{kyn} \todo{cite dovciak}. Line profiles
capture the Doppler shift, special relativistic beaming, gravitational redshift,
and lensing of a particular accretion model of a black hole, but must therefore makes assumptions about the disc geometry or velocity
structure -- assumptions that may differ from the rest-frame models they are
convolved with.
For example, all popular line profile models assume a razor-thin disc in
the equatorial plane with strictly Keplerian velocity structure
\citep[e.g.][]{dovciak_extended_2004, beckwith_iron_2004, brenneman_constraining_2006,
dauser_broad_2010}.
...
\revisit{
We note there are many interesting and informative examples of GRRT with
accretion disc simulations made using general relativistic magneto-hydrodynamics
(GRMHD) codes \citneeded. However, GRMHD simulations struggle with thin disc
models, such as the Shakura-Sunyaev disc, and are expensive
to compute, leading to sparse exploration of the possible parameter spaces.
Since our aim is to create spectral models that can be used to for parameter
inference from spectral data, we omit further discussion of GRRT for GRMHD.}

Recently, there has also been increased interest in ``tests of relativity'',
studying solutions to the Einstein field equations that deviate from a Kerr
black holes (e.g. \citealt{johannsen_testing_2010,
chrusciel_stationary_2012, bambi_testing_2022, patra_accretion_2023,
chen_observational_2024},). This include spacetime metrics that contradict the
\textit{no-hair} uniqueness theorems\footnote{In the theorem, `hair' is metaphor for additional information in
the spacetime beyond mass, angular momentum, and charge.}. This interest is in
part driven by observations made by the
Event Horizon Telescope collaboration of the `shadow' and
photon ring of M87 and Sgr A*
\citep{the_event_horizon_telescope_collaboration_first_2019,
the_event_horizon_telescope_collaboration_first_2023}. These observations have
produced a wealth of study into modified spacetimes
\citep[see][]{eht_non_kerr_2022}. There
is similarly much research dedicated to using disc spectra and line
...
\citep[e.g.][]{bambi_precision_measuremets_2021}.

For most relativistic spectral models, the assumptions made about
the accretion disc and spacetime become embedded in a set of \textit{transfer functions} -- tabular
data used to efficiently calculate line profiles or convolution kernels -- which must be recomputed if
the assumptions are changed. There are few immediately available public codes
that can reliably calculate new transfer function tables, even for the simplified
razor-thin Keplerian disc models, and
none that the authors are aware of that can reliably do so for other disc geometries,
velocity structures, and spacetimes. It is to address this gap that we present a new publicly
available GRRT
code, Gradus.jl\footnote{Open-source and available under GPL 3.0 license at
\url{https://github.com/astro-group-bristol/Gradus.jl}.}. Written in the Julia
programming language \citep{Bezanson_Julia_A_fresh_2017}, our package
is a general toolkit for including relativistic effects in spectral models.
Models created with \Gradus can be used in spectral fitting
packages, such as XSPEC \citep{arnaud_xspec_1996} or SpectralFitting.jl
\citneeded.
...
function of radius, $\varepsilon(\rho)$, for an axis-symmetric system. This is
often referred to as the \emph{emissivity profile} as the line strength in the
back-scattered spectrum is modelled to be proportional to the illuminating flux.
The illumination profile is related to the ionization parameter, $\xi(r)$, of
the accretion disc \citep{laor_line_1991,ross_reflection_1993,
wilkins_understanding_2012} given by
\begin{equation}
    \xi = \frac{4 \pi F_i}{n_\text{H}}
\end{equation}
where $F_i$ is the total illuminating X-ray flux in some band, often $0.01 - 100
...
\cite{cackett_reverberation_2021} for a review). Reverberation lags difficult to
...
spacetime as a means of testing the ``no-hair theorem'' (for a review, see
\citealt{bambi_testing_2023}). \Gradus allows new spacetime metrics to be
implemented easily, and self-consistently calculates higher-order results within
the spacetime, making the software ideal for quickly computing tests for a given
spacetime.

To test our implementation, we compare the line profiles calculated with \Gradus
for the Johannsen spacetime \citep{johannsen_regular_2013} to the line profiles
calculated using the \texttt{rellinenk} of \citneeded. \todo{a comparison
figure}.
...
self-obscuration of the disc has here the same effect as in the linesprofiles;
with steeper inclination, the highest blue shifted component moves to lower
energies. In tandem with this is a decrease in the time lag relative to the
thin disc case, which is comparable to the thickness of the disc. At lower
inclinations, the effect of the disc thickness is negligible. \todo{compare
with results of Taylor and Reynolds and Abdikamalov}.
...
Fourier frequencies. The distant regions of the accretion disc have their
contributions to the observed reflection flux suppressed, as the corona is
obscured by the disc. The effect of lamppost height is pronounced, and can
significantly change the shape of the lag-energy profile without shifting the
...
This example application will hopefully persuade the reader of the importance of
modelling the full complexities of the relativistic system, as small
modifications to the underlying assumptions of the system can have dramatic
consequences for certain parameter combinations of the model. This is not only
true for the particulars of the disc geometry; the coronal geometry similarly
will have marked effect on spectral features. \Gradus is designed to tackle
extended corona modelling using a decomposition technique (\todo{Baker \& Young,
in prep.}).

The spacetime under consideration may similarly exhibit dramatic changes in the
spectral model, with potentially some tell-tale signature that would allow a
particular spacetime to be distinguished from another to high accuracy.
Currently, the reflection profile techniques used to disambiguate one metric
from another often run into strong parameter degeneracies (in particular when
the ISCO of two spacetimes coincide). By expediting the modelling process for
computing observables of new spacetimes, \Gradus has a direct application for
studying modified gravity theories, and can be used to compute convolution
models or (reverberation) timing properties with little additional effort.

An anticipated application for \Gradus is to include the treatment of polarization
in the software. This would allow the Stokes parameters to be traced through the
simulation, and for polarization models to be efficiently computed using the
same extension to the Cunningham transfer function approach as we demonstrated
with timing in this work.
