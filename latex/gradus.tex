...
grid in $r$ should be irregularly spaced, e.g. $\sim 1 / r$, whereas the grid in
$g$ should be linear. This reflects that the majority of the variation in $f$
occurs at small $r$.}.

Most of the variation in the Cunningham transfer functions occurs at small
$r_\text{em}$, and so we calculate Cunningham transfer functions over a power
series of $r_\text{em}$ between the extrema. For $g^\ast$, the variation is
approximately even, and so we use a uniform grid of $N = 30$ points over
$g^\ast$. This also make saving the transfer functions in a tabular format
simple.
