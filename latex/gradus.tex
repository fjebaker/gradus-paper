...
accounting for the full relativistic effects of strong gravity.

The GRRT approach has a long history in spectral modelling, and sees widespread
use in models of reflection spectra \citep[e.g.][]{fabian_x-ray_1989}, the X-ray
corona and accretion disc emissivities
...
The standard method for including relativistic effects calculated using GRRT
...
\cite{karas_light_1992} and \cite{karas_vicinity_1992} examine the changing
line profiles of hot spots eclipsed in thick discs, \cite{pariev_line_1998} study
the effects of turbulent velocities in a Novikov-Thorne accretion discs,
...
taylor_x-ray_2018_reverb} calculate tabular line profile models and reverberation
features for a disc with the Shakura-Sunyaev height profile,
\cite{thomsen_relativistic_2022} model X-ray reverberation in super-Eddington
accretion flows, and \cite{abdikamalov_testing_2020} study a Novikov-Thorne disc
in the \cite{johannsen_regular_2013} spacetime.
...
and telescopes with larger collecting areas, higher resolution, and more
flexible models are needed. Today, it is the advent of XRISM
\citep{tashiro_xrism_2020} and expected launch of NewAthena
\citep{barret_athena_2013} that is driving the need for more detailed models,
and, by extension, more considerate treatment of the relativistic effects. At
near eV resolution, model noise may skew the fit statistic due to the presence
of hitherto unresolved narrow spectral features.  It is not only that some
existing models must be computed higher precision, but that many models need
their underlying assumptions re-evaluated, as subtle effects that were once
deemed too difficult to distinguish may now be discoverable -- including
particulars of the accretion structure.  These include  accretion models that
have small scale variations, such as spiraling hot spots, warped geometrical
structure, or processions, and for observations with high signal-to-noise with
relatively narrow emission and absorption features. \todo{explain more?}
...
are being convolved with; from disc and coronal geometry, to absorptive and
emissive features. The work we present in this paper is towards providing the
tools and techniques to address these needs. We present a new open-source GRRT
code, \Gradus\footnote{Open-source and available under GPL 3.0 license at
...
publicly available in the code and detailed in the documentation. In Section
...
calculated using the \texttt{relline_nk} of \citneeded. \todo{a comparison
figure}.
