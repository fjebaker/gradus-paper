...
of the system. These effects are easiest to calculate using
general relativistic ray-tracing (GRRT) software.

GRRT has a long history in spectral modelling. These codes see widespread use in
studying the broad iron lines, emission and absorption lines, reverberation
signatures, AGN SED, caustics, quasi-periodic oscillations, TDEs, winds.
...
lineprofile, using models such as \texttt(relxill} or its variants. This blurs
the rest-frame emission and accounts for the doppler shift, relativistic
beaming, and the gravitational redshift. These convolution models may make
assumptions that differ from the model it is being convolved with. The
assumptions are in the tabular data used to integrate the lineprofile, the
\textit{transfer functions}, of which there are few public codes that can
