...
\citep{cunningham_optical_1973}, timing properties
\citep{stella_measuring_1990}, and appearances of the systems
...
calculated using GRRT is to convolve the rest-frame spectrum with a relativistic
lineprofile \citep{laor_line_1991}, using models such as \texttt{relline} /
...
beaming, and the gravitational redshift relative to a distant observer and
sensitive to the spacetime solution. The
convolution models, however, make assumptions that may differ from the models
they are being convolved with; most fundamentally in the geometry of the
accretion disc. The majority of convolution models assume a razor-thin disc
structure (\citealt{dovciak_extended_2004, brenneman_constraining_2006,
dauser_broad_2010}), with the notable exceptions being the models of
\cite{taylor_exploring_2018} and \cite{abdikamalov_testing_2020}, where the
Shakura-Sunyaev disc (SSD) structure is used. The assumptions are encoded in the
transfer functions -- the tabular data used to integrate calculate the
lineprofile -- for which which there are few public codes that can calculate new
...
% and continuing the tradition of iterating higher resolution
