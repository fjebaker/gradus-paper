...
The presence of an illuminating source near the accretion disc, such as a corona
\citep{svensson_corona_1994}, will induce an \emph{illumination profile} on the
disc. It is equivalently the \emph{emissivity profile}, provided the line
strength in the back-scattered (reflected) spectrum is modelled to be
proportional to the illuminating flux. The illumination profile is the flux
deposited by the source in a patch of the disc
\citep{wilkins_understanding_2012}. It may be expressed as the ratio of source
solid angle area to disc area, or in the limiting case as the areas become
small,
\begin{equation}
    \varepsilon \propto I \frac{\d \Omega}{\d A}.
\end{equation}
The illumination profile is also related to the ionization parameter $\xi$ of
the accretion disc \citep{laor_line_1991,ross_effects_1993}, itself given by
\begin{equation}
    \xi = \frac{4 \pi F_i}{n_\text{H}}
\end{equation}
where $F_i$ is the total illuminating X-ray flux in a particular energy band
$i$, usually in the range $0.01 - 100 \text{ keV}$, and $n_\text{H}$ is the
co-moving hydrogen number density \citep{ross_effects_1993}. This formulation
assumes constant density throughout the disc. For our purposes, we will drop
...
geometries have been computed in \citet{gonzalez_probing_2017} and others
\todo{cite all sorts of papers}. These methods
...
\begin{equation}
    ...
    \d F(E) = I(E)\, \d \Omega,
\end{equation}
for observed energy $E$ and specific intensity $I$. The relation between the
...
To obtain the flux as measured by the observer, we integrate over the observer
field of view$\d \Omega$. This is up to a constant equivalent to integrating
over the image plane $\d \alpha \d \beta$,
\begin{equation}
\label{eq:integrate-impact-params}
F(E_\text{obs}) \propto \int I_\text{obs}(E_\text{obs}) \d \alpha \d \beta,
\end{equation}
which in practical terms is binning the geodesic associated with each pixel into
a grid of $E$. Formulating the flux calculation in this way is computationally
...
to be stored in memory. These quantities are dependant on the metric, disc, and
observer parameters, and this dependence limits the reuse of calculations
between simulations -- an undesirable property for a portable spectral model.
...
cosine angle $\mu := \cos x^\theta$ to the disc surface.
...
lag--energy profiles exists for all inclinations and frequencies. While these
differences may be observable with current data, their limited signal-to-noise
ratios would likely result in degeneracies with other fitted parameters
including black hole mass (e.g., current observations may not be able to cleanly
separate the timing effects of changing disc geomety and varying black hole
mass). We note that this effect can also be attributed to the geometry of the
corona \citep[see e.g.][]{wilkins_towards_2016}.
