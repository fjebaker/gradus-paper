...
General relativistic effects must be included when modelling the innermost
regions of accretion onto compact objects. The relativistic effects and
spacetime curvature in the strong gravity regime lead to significant departures
from the classical picture, in the energetics, timing properties, and appearance
of the system. These effects are easiest to calculate using general relativistic
ray-tracing (GRRT) software. The GRRT approach has a long history in spectral
modelling, and sees widespread use in models of reflection spectra, broad iron
lines, emission and absorption features including winds, for variability studies
such as reverberation lags, TDEs, and quasi-periodic oscillations (QPOs). The
use of the GRRT approach has enabled the measurement of black hole spins using
the broad iron line, and is at the forefront of modelling the X-ray corona.

The standard approach for including the effects of general relativity (GR) in a
spectral model is to convolve the rest-frame spectrum with a relativistic
lineprofile, using models such as \texttt{relxill} and its variants. These
convolution models blur the rest-frame emission thereby accounting for the
doppler shift, relativistic beaming, and the gravitational redshift, amongst
other effects. These models, however, may make assumptions that differ from the
model it is being convolved with. The assumptions are in the tabular data used
to integrate the lineprofile, the \textit{transfer functions}, of which there
are few public codes that can calculate new tables.

Due to the computational complexity associated with GRRT, relativistic models
may trade resolution for performance. This has been generally acceptable
practice, as the noise in the models is generally at a resolution that is
blurred out when folded through an instrument response matrix. With
the advent of high resolution spectrometers, such as XRISM and NewAthena, this
is no longer the case, and there
is a need for suitably high resolution spectral models. At near sub-eV
resolution, model noise may skew the fit statistic.

The work we present in this paper is a new open-source GRRT,
...
programming language. The software aims to be a toolkit for including
relativistic effects in spectral models. The software is designed to efficiently
calculate transfer functions for new convolution models. To allow
the models that \Gradus calculates to be used in popular spectral fitting
suites, such as XSPEC, we also maintain C-ABI compatible wrappers\footnote{We
    use the Zig programming language to wrap the spectral models, as it allows
us to statically compile models for a wide variety of targets. Models can then
be downloaded and imported into e.g. XSPEC without the need for any additional
tools.}. \Gradus then allows the researcher to construct their own convolution
models that more closely match the assumptions they make in their models with
respect to accretion disc geometry, velocity structure, corona, and so on.
Additionally, \Gradus is designed to be extensible, so that it may be used to
include the relativistic effects directly in the rest frame calculations of
other models.

- it is especially important today that we get the GR effects right because of
how good the data is about to be

In recent years there has been increased interest in testing the Kerr
hypothesis, and studying solutions to the Einstein field equations that deviate
from a spacetime parameterised by the mass and angular momentum of the
singularity. This interest has been in part driven by the results published by
the Event Horizon Telescope collaboration in studying the event horizon `shadow'
and photon rings from radio interferometry data. In the reflection spectra
communities, the potential for the broad iron line to distinguish between
spacetime solutions has yielded results and seemingly tight constraints on
deformation parameters for some solutions.
...
We have created a toolkit for modelling the effects of general relativity in a
wide variety of contexts. The toolkit is developed in such a way as to support a
component modelling approach, such that individual models (spacetimes, coronae,
accretion discs, analysis procedures) can be developed separately and combined
into ensemble models that include full relativistic treatment. Our software aims
to be a sandbox for creating spectral models, and for rapidly implementing and
testing new models and theories.
...
\notes{
In the era of quantitative, precision observational tests of General Relativity in the strong field regime it is necessary to have a fast and flexible method to compute the observational properties of accreting black hole systems.
We have developed an open-source integrator
\Gradus\footnote{Open-source and available under GPL 3.0 license at \url{https://github.com/astro-group-bristol/Gradus.jl}.} for this purpose.

In the remainder of the paper we describe how the software works, comparing with previous work in the literature, and outlining the new capabilities of \Gradus.
...
The paper is organised as follows: in Section \ref{sec:numerical-methods} we
detail the numerical methods used in \Gradus, introducing the approaches and
algorithms used in the code. In Section \ref{sec:description-of-code} the
software is described, with a focus on the organisation of the code and its
extensibility. Section \ref{sec:test-problems} the validity of the code is
assessed for a number of standard problems in the literature, and compared to
other published codes and results. Intended applications of and future work on
the code are described in \ref{sec:applications}.
