...
departures from the classical picture, from the energetics
\citep{cunningham_optical_1973, fabian_long_2002}, timing properties
\citep{stella_measuring_1990, reynolds_x-ray_1999}, and appearances of the systems
\citep{luminet_image_1979}. These effects are often included in models through
the use of general relativistic ray-tracing (GRRT) software. The GRRT approach
has a long history in spectral modelling, and sees widespread use in models of
reflection spectra (e.g. \citealt{fabian_x-ray_1989}), accretion disc
emissivities and the X-ray corona (e.g. \citealt{wilkins_understanding_2012, wilkins_towards_2016}) emission and absorption features including
winds, spectral energy densities of quasars (e.g.
\citealt{hagen_estimating_2023}), for variability studies such as reverberation lags (e.g.
\citealt{ingram_public_2019, cackett_reverberation_2021}),
quasi-periodic oscillations (QPOs, e.g. \citealt{tsang_iron_2013}), and so on.

\todo{a little about what GRRT is}
...
\citep{laor_line_1991}, using models such as \texttt{relline} / \texttt{relxill}
\citep{dauser_broad_2010, dauser_relativistic_2016}  and its variants. These
lineprofiles are usually convolution models that blur the rest-frame emission,
thereby accounting for the doppler shift, relativistic beaming, and the
gravitational redshift relative to a distant observer. The convolution models,
however, make assumptions that may differ from the models they are being
convolved with; most fundamentally in the geometry of the accretion disc. The
majority of convolution and lineprofile models assume a razor-thin disc
structure (e.g., \citealt{dovciak_extended_2004, brenneman_constraining_2006,
dauser_broad_2010}), with some notable exceptions: the models of
\cite{karas_light_1992, karas_vicinity_1992} examine hot spots eclipsed in thick
discs, \cite{pariev_line_1998} studies the effects of turbulent accretion in
Novikov-Thorne accretion discs, \cite{taylor_exploring_2018} who calculate
tabular lineprofile models for the Shakura-Sunyaev accretion disc, and
\cite{abdikamalov_testing_2020} where thick disc models are applied to the
Johannsen-Psaltis metric \todo{check this is the right metric}. In the
convolution models, the assumptions made about the become encoded in the
\textit{transfer functions} -- the tabular data used to efficiently calculate
lineprofiles -- which must be recomputed if the assumptions are changed. There
are few immediately available public codes that can reliably and efficiently
calculate new transfer function tables.
...
statistic. With the advent of higher resolution spectroscopic datasets, such as
observation from XRISM and NewAthena \citneeded, this is no longer the case, and
there is once again a need for suitably high resolution spectral models, and, by
extension, high resolution relativistic convolution models. At near sub-eV
resolution, model noise may skew the fit statistic due to the presence of
hitherto unresolved narrow spectral features. High resolution spectroscopy gives
rise to unprecedented opportunities for studying the details of accretion at the
horizon scales\citneeded, which mandates including all relativistic effects.  And `high
resolution' is here not only to mean computing the existing models to higher
precision, but also re-evaluating the underlying models themselves\citneeded, as subtle
effects that were once deemed too small to measure ought now be included.
...
Section \ref{sec:applications}. Finally, overall conclusions of the work are discussed
...
$\utensor{\Gamma}{\mu}{\nu\sigma}$ \textit{on the fly}\footnote{We were made
    aware by colleagues of the Mahakala code of \citet{sharma_mahakala_2023}
    which uses the same AD approach,
    indicating that the method is indeed sufficiently accurate and performant
for GRRT needs.}. This brings
