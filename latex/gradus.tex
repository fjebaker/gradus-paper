...
calculating the Cunningham transfer functions is modified through the use of
\emph{datum planes}.
...
including also those geodesics from obscured regions of the disc. As the
redshift depends only on the start and end point of the geodesic, it is
therefore simpler to combine this with the radius-solving step and calculate the
redshift over the datum plane. This is not so for the Jacobian term, which
depends on the cross section traced by a bundle of geodesics for which the
geometry of the disc is important. The Jacobian calculating step is combined
with a check to see if the patch of the disc is obscured or not, which is then
used to mask the transfer functions, as in Figure \ref{fig:transfer-functions}.

This method is analogous to the ``imaginary photons'' of
\cite{abdikamalov_testing_2020} formalized for optimizing $r_\text{em}$ and use
with AD.
...
The Cunningham transfer functions may be viewed as the Green's function of an
annulus $r_\text{em}$ in response to some delta function of incident flux.
...
\todo{Mention that the extension to using the angle of emission is trivial with
