...
Using coordinates $x^\mu$, the geodesic equation with under external
...
\label{eq:christoffel}
    ...
    \right),
\end{equation}
with the indices to be read as ``the acceleration in direction $\mu$ of the vector
...
implementation of GRRT, as it becomes a factor when determining dot products
velocity vectors.
...
Following \cite{cunningham_optical_1973}, one may then define a set of \emph{impact
...
The case for emitters is subtly different, where we now consider azimuthal and
polar angles in the local sky, denoted $\Upsilon$ and $\Psi$ respectively (see
